...
- 📋 **一键复制结果** — 链接 / 文字两种格式
- 🖼️ **保存分享海报** — 结果页一键导出 PNG（1200×1500），便于发朋友圈 / 小红书
- 📲 **PWA** — `manifest.json` + Service Worker，可「添加到主屏幕」，弱网或离线仍可打开已缓存页面
- 🌗 **响应式设计** — 桌面 / 平板 / 手机适配
- 🚫 **零依赖** — 单个 HTML 文件，无构建，无后端（PWA 仅增加静态 `manifest.json` / `sw.js` / `icon.svg`）
